...
const confirmExceptions = (
  ...
  exceptions.forEach((date) => {
    if (
      new Date(date).getDate() === new Date(dateToConfirm).getDate() &&
...
const EventsGenerator = (
  ...
  const eventsFullList = [{} as CalendarEvent];
  ...
  events.forEach((event: eventInterface) => {
    ...
    if (!event.repeatNeverEnds && event.repeat !== 'None') {
      const dateStartCount = new Date(event.start);
      const dateEndCount = new Date(event.end);
    ...
    if (event.repeatNeverEnds) {
      ...
      const eventStartCount = new Date(event.start);
      const eventEndCount = new Date(event.end);
